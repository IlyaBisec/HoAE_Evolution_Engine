...
public:
	...
	double * Ptr();
	friend cStr & operator << (cStr &, const cVector &);
};
...
inline void cVector::Copy(const cVector &Src) {
	m_dim = Src.m_dim;
	x = Src.x;
	y = Src.y;
	z = Src.z;
...
// cVector::Normal
//-----------------------------------------------------------------------------
inline cVector cVector::Normal() const {
	cVector t(*this);
	return t.Normalize();
} // cVector::Normal
...
// Spherical
//-----------------------------------------------------------------------------
inline void cVector::Spherical(double &r, double &Phi, double &Psi) const {
	r = Length();
	if(cUtils::IsZero(r)) {
		r = 0;
	} else {
		Psi = acos(y / r);
		double l = sqrt(Square(x) + Square(z));
		if(cUtils::IsZero(l)) {
			Phi = 0;
		} else {
			if(z >= 0) Phi = asin(x / l);
			else Phi = cPi - asin(x / l);
			AngleNormalize2Pi(Phi);
		}
	}
}
