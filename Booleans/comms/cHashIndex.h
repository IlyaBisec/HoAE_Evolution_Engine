#ifndef __CHASHINDEX__
#define __CHASHINDEX__
...
#define cHashIndex_defHashSize		1024
#define cHashIndex_defGrowIndexBy	1024
...
public:
	...
	int GenerateKey(const char *string, bool CaseSensitive) const;
	int GenerateKey(const cVector &v) const;
private:
...
inline cHashIndex::cHashIndex() {
	Init(cHashIndex_defHashSize, cHashIndex_defGrowIndexBy);
}
...
inline int cHashIndex::GenerateKey(const char *string, bool CaseSensitive) const {
	if(CaseSensitive) return cStr::Hash(string) & m_HashMask;
	else return cStr::CaseInsHash(string) & m_HashMask;
}

// cHashIndex::GenerateKey(const cVector &)
inline int cHashIndex::GenerateKey(const cVector &v) const {
