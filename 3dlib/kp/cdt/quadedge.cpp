...
public:
	Real					x, y;
	c2::array<HashEdge*>	edges;
		
	unsigned int hash() const
	{
		unsigned char* s = (unsigned char*) this;
		unsigned int h = 0;
		int nBytes = sizeof( HashVert );
	...
	bool equal( const HashVert& el )
	{
		return (fabs( x - el.x ) + fabs( y - el.y ) < Mesh::s_VertexWeldTreshold);
	} // equal

	bool operator ==( const HashVert& vert )
	{
		return equal( vert );
	}

	void copy( const HashVert& el )
	{
		memcpy( this, &el, sizeof( HashVert ) );
	} // copy
...
public:
	...
	bool equal( const HashPoly& el )
	{
		return  (v1 == el.v1 && v2 == el.v2 && v3 == el.v3) ||
				(v1 == el.v2 && v2 == el.v3 && v3 == el.v1) ||
				(v2 == el.v3 && v3 == el.v1 && v1 == el.v2) ||
				
				(v2 == el.v1 && v3 == el.v2 && v1 == el.v3) ||
				(v3 == el.v2 && v1 == el.v3 && v2 == el.v1) ||
				(v3 == el.v3 && v1 == el.v1 && v2 == el.v2);
				
	} // equal

	bool operator ==( const HashPoly& poly )
	{
		return equal( poly );
	}
...
public:
	...
	bool equal( const HashEdge& el )
	{
		return  (v1 == el.v1 && v2 == el.v2) ||
				(v1 == el.v2 && v2 == el.v1);
	} // equal

	bool operator ==( const HashEdge& poly )
	{
		return equal( poly );
	}

	void set( int a, int b ) { v1 = a; v2 = b; }

	static int AngleCmp( const void* pE1, const void* pE2 )
	{
		HashEdge* e1 = *((HashEdge**) pE1);
		HashEdge* e2 = *((HashEdge**) pE2);
		
		int v1 = e1->v1; 
		int cv = e1->v2;
		if (v1 == e2->v1 || v1 == e2->v2) 
		{
			cv = v1;
			v1 = e1->v2;
		}
		...
		int v2 = e2->v1; 
		if (v2 == e1->v1 || v2 == e1->v2) v2 = e2->v2;
		
		Real x1 = pVHash->Elem( v1 ).x;
		Real y1 = pVHash->Elem( v1 ).y;

		Real x2 = pVHash->Elem( v2 ).x;
		Real y2 = pVHash->Elem( v2 ).y;

		Real cx = pVHash->Elem( cv ).x;
		Real cy = pVHash->Elem( cv ).y;
		
		Real ang1 = atan2( y2 - cy, x2 - cx );
		Real ang2 = atan2( y1 - cy, x1 - cx );

		if (ang1 < ang2) return 1;
		if (ang1 == ang2) return 0;
		return -1;
	}

	static LinearRegistry<HashEdge>*	pEHash;
	static LinearRegistry<HashVert>*	pVHash;

}; // class HashEdge

Real Mesh::s_VertexWeldTreshold = EPS;

LinearRegistry<HashEdge>*	HashEdge::pEHash = NULL;
LinearRegistry<HashVert>*	HashEdge::pVHash = NULL;


void Mesh::CreateIndexedMesh(	void* vbuf, int& nVert, int vstride, 
								unsigned short* ibuf, int& nInd )
{
	LinearRegistry<HashVert>	vhash;
	LinearRegistry<HashEdge>	ehash;
	LinearRegistry<HashPoly>	phash;

	HashEdge::pEHash = &ehash;
	HashEdge::pVHash = &vhash;

	for (LlistPos p = edges.first(); !edges.isEnd(p); p = edges.next(p)) 
	{
		Edge *e = (Edge*)edges.retrieve(p);
		
		HashEdge he; 
		HashVert hv1, hv2;
		
		hv1.set( (Real)e->Org()->x, (Real)e->Org()->y );
		hv2.set( (Real)e->Dest()->x, (Real)e->Dest()->y );
		
		int v1 = vhash.Add( hv1 );
		int v2 = vhash.Add( hv2 );
	...
	for (int i = 0; i < ehash.GetNElem(); i++)
	{
		HashEdge& edge = ehash.Elem( i );
		vhash.Elem( edge.v1 ).edges.push_back( &edge );
		vhash.Elem( edge.v2 ).edges.push_back( &edge );
	}
	
	for (int i = 0; i < vhash.GetNElem(); i++)
	{
		HashVert& v = vhash.Elem( i );
		int nEdges = v.edges.size();
		qsort( &(v.edges[0]), nEdges, sizeof( HashEdge* ), HashEdge::AngleCmp );
		for (int j = 0; j < nEdges; j++)
		{
			HashEdge& e1 = *(v.edges[j]);
			HashEdge& e2 = *(v.edges[(j + 1)%nEdges]);

			int v1 = e1.v1; if (v1 == i) v1 = e1.v2;
			int v2 = e2.v1; if (v2 == i) v2 = e2.v2;
			...
			Real x1 = vhash.Elem( v1 ).x;
			Real y1 = vhash.Elem( v1 ).y;

			Real x2 = vhash.Elem( v2 ).x;
			Real y2 = vhash.Elem( v2 ).y;

			Real cx = vhash.Elem( i ).x;
			Real cy = vhash.Elem( i ).y;

			Real ang1 = atan2f( y2 - cy, x2 - cx );
			Real ang2 = atan2f( y1 - cy, x1 - cx );
			Real dang = ang1 - ang2;
			if (dang < 0) dang += c_PI*2.0f;
			if (fabs( dang ) >= c_PI) 
			{
				continue;
			}
