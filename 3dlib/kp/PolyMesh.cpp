...
Vector3D Hermit(Vector3D V1,Vector3D N1,Vector3D V2,Vector3D N2,float s,Vector3D& N){
...
bool PolyMesh::ReadObj(const char* Name){        
	...
	for(int i=0;i<nsubdiv;i++){
		...
		for(int q=0;q<nf;q++){
			DivideFace(q);
			int nf2=FacesInfo.GetAmount();
			nf2=FacesInfo.GetAmount();
		}		
	}	    
                ...
                if(ins==0xFFFFFFFF){
                    ...
                    if(P==D){
                        ...
                        if(cN!=PrevN){                        
							...
							nc.normalize();							
							Vector3D v2=Hermit(Verts[uv1->PosIndex].Pos,N1,Verts[uv2->PosIndex].Pos,N2,0.5f,nc);
    ...
    if(nf>0){
		...
		if(OrigVerts.GetAmount()&1){
			...
			for(int i=0;i<n2;i++){
				...
				Vector3D N;
				summ+=Hermit(Verts[uv1->PosIndex].Pos,VertsN[uv1->NIndex],Verts[uv2->PosIndex].Pos,VertsN[uv2->NIndex],0.5f,N);
				summN+=N;
			}
			summ/=n2;
			summN.normalize();
...
void OneBackupItem::Undo(){
    for(int i=0;i<Backup.GetAmount();i++){
        ...
        if(FI->Points){
            memcpy(FI->Points,FB->Points,M->SubSurfaceSize*M->SubSurfaceSize*sizeof SurfPoint3D);
        }
		int p=0;
...
void OneBackupItem::Save(BinStream* bs){    
    ...
    for(int i=0;i<Backup.GetAmount();i++){
        ...
        if(fb->Points){
            bs->Write(fb->Points,M->SubSurfaceSize*M->SubSurfaceSize*sizeof(SurfPoint3D));
        }        
    }
}
void OneBackupItem::Load(BinStream* bs){
    ...
    for(int i=0;i<n;i++){
        ...
        if(FB->Points){
            FB->Points=new SurfPoint3D[M->SubSurfaceSize*M->SubSurfaceSize];
            bs->Read(FB->Points,M->SubSurfaceSize*M->SubSurfaceSize*sizeof(SurfPoint3D));
        }
        Backup.Add(*FB);
    }
}
void PolyMesh::FreeUndoHistory(){
	...
	UndoHistory.Clear();
	scan(FacesHistory,OneFaceBackup** B,DWORD* Face){
        delete(*B);
	}scan_end;
	FacesHistory.reset();
}
SurfPoint3D* PolyMesh::GetFaceHistory(DWORD Face){
	OneFaceBackup** B=FacesHistory.get(Face);
...
void PolyMesh::DropFaceToUndo(DWORD Face){
	...
	if(!B){
		if(FacesInfo[Face].Points){
			...
			FB->Points=new SurfPoint3D[n];
			memcpy(FB->Points,FacesInfo[Face].Points,n*sizeof SurfPoint3D);
			int p=0;
			scan_key(Faces,Face,DWORD* Vert){
				VertexUV* uv=&VertsUV[*Vert];
				if(p<4){
...
void SurfaceModificator::CreateVertList(){
	NearVerts.FastClear();
	NearFaces.FastClear();
	InsideVerts.FastClear();
	if(Pick->Picked){
...
void PolyMesh::CreateFaceNormals(DWORD Face){
    FaceInfo* fi=&FacesInfo[Face];
    if(fi->Points){
        int ofs=0;
        PointInfo pinf;        
		...
		for(int dx=-1;dx<=0;dx++){
			for(int dy=-1;dy<=0;dy++){        
				PointInfo p0=GetPoint(Face,dx,dy);
				v[0]+=p0.pt->Pos;
				n[0]+=p0.pt->N;
				p0=GetPoint(Face,SubSurfaceSize+dx,dy);
				v[1]+=p0.pt->Pos;
				n[1]+=p0.pt->N;
				p0=GetPoint(Face,SubSurfaceSize+dx,SubSurfaceSize+dy);
				v[2]+=p0.pt->Pos;
				n[2]+=p0.pt->N;
				p0=GetPoint(Face,dx,SubSurfaceSize+dy);
				v[3]+=p0.pt->Pos;
				n[3]+=p0.pt->N;
			}
		}		
...
void SurfaceModificator::ApplyModFunfcion(fnModificator* fn,bool Undo){
	...
	for(int i=0;i<NearFaces.GetAmount();i++){		
		...
		if(fi->Points){
			...
			SurfPoint3D* pi0=M->GetFaceHistory(Face);
            if(!pi0)pi0=pi;
			mpi.Curr=pi;
			mpi.Orig=pi0;
			mpi.Face=Face;
			...
			if(pi0){
				...
				for(int y=0;y<sz;y++){
					for(int x=0;x<sz;x++){						
						...
						mpi.y=y;
                        float w=0;
						Vector3D D=GetDVToRode(pi0->Pos*M->Scale,w);                        
						...
						if(dz>0){
							if(w>0 && w<1){
								mpi.StrokeLength=sl/R1*1.2;
							}else mpi.StrokeLength=0;
							if(w==0){
								float w1;
								Vector3D D1=GetDVToRodeP(pi->Pos*M->Scale,w1);
								if(w1==1){
									mpi.StrokeLength=0.5;
									float r=sqrt(mpi.dx*mpi.dx+mpi.dy*mpi.dy);
									mpi.dx=0;
									mpi.dy=mpi.dy<0?-r:r;
								}
							}						
			...
			if(pi0){
				...
				for(int y=0;y<sz;y++){
					for(int x=0;x<sz;x++){						
						...
						mpi.y=y;
                        float w=0;
						Vector3D D=GetDVToRode(pi->Pos*M->Scale,w);                        
                        float R1=PrevR*(1-w)+Radius*w;
        ...
        if(sz==5){
            ...
            for(int y=0;y<size;y++){
                ...
                for(int x=0;x<size;x++){
                    ...
                    sp[ps].Color = V4D2DW(Texture.GetPixel(uu,vv));
                    sp[ps].Freeze=0;
                    ps++;
                }
            }
		...
		do{
			...
			if(Mg=='SURF'){				
				...
				for(int i=0;i<nfc;i++){
					...
					st.Read(fi->Points,SubSurfaceSize*SubSurfaceSize*sizeof SurfPoint3D);
					int vl=-1;
					scan_key(Faces,i,DWORD* Vert){
						VertexUV* uv=&VertsUV[*Vert];        
						if(vl!=-1){
...
void TestObj(){	
    ...
    if(PMS.LastPickInfo.Picked){ 		
		...
		sm->Pressure=1.0f;
		if(WT_IS_INIT && GeneralPen::PenPressureType!=2 && GeneralPen::PenPressureType!=3){
			if(PenPressed){
		...
		for(int i=0;i<sm->NearVerts.GetAmount();i++){
            PickedVertInfo& pi=sm->NearVerts[i];
			//if(pi.DistToCenter<sm.Radius){
			VertexBackup* VB=SurfaceModificator::Backup.get(pi.PosVertexID);
			if(VB){
				DWORD R=pi.DistToCenter*255/sm->Radius;
				DrawSphere(Sphere(VB->Pos*PMS.Scale,0.3),0xFF000000+(R<<16),0,4);					
			}
			//}
		}
		*/
		...
		GeneralPen::RenderOnPen(PMS.LastPickInfo.Pos,PenR);		        
						...
						for(int y=0;y<=ym;y++){
							for(int z=0;z<=zm;z++){
								if(x||y||z){
									...
									GeneralPen::TransformSymm(PMS.LastPickInfo.Pos,mul);
									GeneralPen::TransformSymm(BasicTool::Pick.PickPos,mul);
									GeneralPen::TransformSymm(SurfaceModificator::PrevPos,mul);
									GeneralPen::TransformSymm(SurfaceModificator::PrevPrevPos,mul);
									GeneralPen::TransformSymm(SurfaceModificator::CurrPos,mul);
									GeneralPen::TransformSymm(SurfaceModificator::CenterPos,mul);
									sm->CreateVertList();
									sm->Apply();
									GeneralPen::TransformSymm(PMS.LastPickInfo.Pos,mul);
									GeneralPen::TransformSymm(BasicTool::Pick.PickPos,mul);
    ...
    int T3=GetTickCount();    
	DrawText(400,0,0x30ffffff,"Trial vertsion - %d minutes remains",(T2-T3)/1000/60);
    try{
	    Widgets::DrawWidgets();
    }catch(...){
        if(T3>T2 && T3<T2+580){
