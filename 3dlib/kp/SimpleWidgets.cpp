...
void Widgets::DrawWidgets(){	
	...
	if(CurrentHint.pchar()[0]){		
		...
		DrawFrame(Rct(0,rct.h-20,rct.w,20),tex,Rct(0,0,1,1),0xFF000090);		
		DrawText(10,rct.h-15,0xFFFFFFFF,"%s",CurrentHint.pchar());		
	}
	FlushText();
}
...
bool SimpleSlider::draw(){
    ...
    float p=(v-MinValue)*Rect.w/(MaxValue-MinValue);
    if(p<0)p=0;
    if(p>Rect.w)p=Rect.w;
    rsRect(Rct(CurrentPos.x,CurrentPos.y,p,Rect.h/2),0,C1,C1,Color,Color);
    rsRect(Rct(CurrentPos.x,CurrentPos.y+Rect.h/2,p,Rect.h/2),0,Color,Color,C1,C1);
    return false;
}
...
bool PictMenu::OnDraw(){
	if(BG && Icons.GetAmount()){
		BG->SetIcon(IconsTexture,Icons[CurrPos],IcNx,IcNy);
	}
	if(FW){
		...
		if(Align==0){//left
			float w=Rect.w;
			float h=Rect.h;
			float ww=Margin*2+n*(w-Margin*2);
            FW->Rect=Rct(-ww,0,ww,h);			
			for(int i=0;i<n;i++){
				DrawTexRect* TR=(DrawTexRect*)FW->SubWidgets[i];
				TR->Rect=Rct(-Margin-(w-Margin*2)*(i+1),Margin,w-Margin*2,h-Margin*2);
			}
		}
		if(Align==1){//right
			float w=Rect.w;
			...
			for(int i=0;i<n;i++){
				DrawTexRect* TR=(DrawTexRect*)FW->SubWidgets[i];
				TR->Rect=Rct(Margin+(w-Margin*2)*i,Margin,w-Margin*2,h-Margin*2);
			}
		}
		if(Align==2){//top
			float w=Rect.w;
