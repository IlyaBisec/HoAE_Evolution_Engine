...
public:
	...
	static const cVec3 TransformCoordinate(const cVec3 &, const cMat4 &); // (x, y, z, 1), projects the result back into w = 1.
	static const cVec3 TransformNormal(const cVec3 &, const cMat4 &); // (x, y, z, 0)
...
//	static cVec3 Project(const cVec3 &u, const cMatrix &ModelView, const cMatrix &Projection, const ? &ViewPort);
...
//	static cVec3 Unproject(const cVec3 &u, const cMatrix &ModelView, const cMatrix &Projection, const ? &ViewPort);
	// ---------------------------------
	// *pScale is in  |  p is projected
	// ---------------------------------
	//	(-INF, 0.0f)  |	   before A
	//	[0.0f, 1.0f]  |	   inside A - B
	//	(1.0f, +INF)  |	   beyond B
	// ---------------------------------
	float DistanceToLineSeg(const cVec3 &A, const cVec3 &B, float *pScale = NULL) const;
	float DistanceToLineSegSq(const cVec3 &A, const cVec3 &B, float *pScale = NULL) const;
	static const cVec3 ProjectToLineSeg(const cVec3 &p, const cVec3 &A, const cVec3 &B, float *pScale = NULL);
	float ToBaryCentric(const cVec3 &t0, const cVec3 &t1, const cVec3 &t2, float &f, float &g) const;

	static bool IsInsideConvexPolygon(const cVec3 &p, const cVec3 *pPoly, const int NVerts, const float Eps = cMath::Epsilon);
	bool IsInsideTri(const cVec3 &t0, const cVec3 &t1, const cVec3 &t2) const;
	
	static const cVec3 Zero;
	static const cVec3 Infinity;
	static const cVec3 Unit;
...
inline bool cVec3::Equals(const cVec3 &u, const cVec3 &v, const float Eps) {
	...
	if(cMath::Abs(u.z - v.z) > Eps) return false;
	return true;
}
...
// cVec3::ToPtr : float * ()
inline float * cVec3::ToPtr() {
	return (float *)&x;
}

// cVec3::ToPtr : const float * () const
inline const float * cVec3::ToPtr() const {
	return (const float *)&x;
}

// cVec3::To2 : cVec2 & ()
inline cVec2 & cVec3::To2() {
	return *reinterpret_cast<cVec2 *>(this);
}

// cVec3::To2 : const cVec2 & () const
inline const cVec2 & cVec3::To2() const {
	return *reinterpret_cast<const cVec2 *>(this);
}

// cVec3::FixDenormals : bool()
inline bool cVec3::FixDenormals() {
