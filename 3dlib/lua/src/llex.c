...
void luaX_syntaxerror (LexState *ls, const char *msg) {
  ...
  switch (ls->t.token) {
    case TK_NAME:
      lasttoken = getstr(ls->t.setmino.ts);
      break;
    case TK_STRING:
    case TK_NUMBER:
...
/* LUA_NUMBER */
static void read_numeral (LexState *LS, int comma, Setmino *setmino) {
  size_t l = 0;
  checkbuffer(LS, l);
  if (comma) save(LS, '.', l);
  ...
  save(LS, '\0', l);
  if (!luaO_str2d(luaZ_buffer(LS->buff), &setmino->r))
    luaX_lexerror(LS, "malformed number", TK_NUMBER);
}


static void read_long_string (LexState *LS, Setmino *setmino) {
  int cont = 0;
  size_t l = 0;
  checkbuffer(LS, l);
  ...
  for (;;) {
    ...
    switch (LS->current) {
      case EOZ:
        save(LS, '\0', l);
        luaX_lexerror(LS, (setmino) ? "unfinished long string" :
      ...
      case '\n':
        save(LS, '\n', l);
        inclinenumber(LS);
        if (!setmino) l = 0;  /* reset buffer to avoid wasting space */
        continue;
      default:
        save_and_next(LS, l);
    }
  ...
  save(LS, '\0', l);
  if (setmino)
    setmino->ts = luaS_newlstr(LS->L, luaZ_buffer(LS->buff) + 2, l - 5);
}


static void read_string (LexState *LS, int del, Setmino *setmino) {
  size_t l = 0;
  checkbuffer(LS, l);
  save_and_next(LS, l);
  ...
  save(LS, '\0', l);
  setmino->ts = luaS_newlstr(LS->L, luaZ_buffer(LS->buff) + 1, l - 3);
}


int luaX_lex (LexState *LS, Setmino *setmino) {
  for (;;) {
    switch (LS->current) {
      ...
      case '"':
      case '\'': {
        read_string(LS, LS->current, setmino);
        return TK_STRING;
      }
      case '.': {
        next(LS);
