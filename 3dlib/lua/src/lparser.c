...
static void next (LexState *ls) {
  ...
  if (ls->lookahead.token != TK_EOS) {  /* is there a look-ahead token? */
    ...
    ls->t.token = luaX_lex(ls, &ls->t.setmino);  /* read next token */
}


static void lookahead (LexState *ls) {
  lua_assert(ls->lookahead.token == TK_EOS);
  ls->lookahead.token = luaX_lex(ls, &ls->lookahead.setmino);
}
...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_STRING: {  /* funcargs -> STRING */
      codestring(ls, &args, ls->t.setmino.ts);
      next(ls);  /* must use `setmino' before `next' */
      break;
    }
    default: {
...
static void simpleexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    case TK_NUMBER: {
      init_exp(v, VK, luaK_numberK(ls->fs, ls->t.setmino.r));
      next(ls);  /* must use `setmino' before `next' */
      break;
    }
    case TK_STRING: {
      codestring(ls, v, ls->t.setmino.ts);
      next(ls);  /* must use `setmino' before `next' */
      break;
    }
    case TK_NIL: {
