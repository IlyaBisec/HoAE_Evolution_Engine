...
//	cVec3 ToAngularVelocity() const;
	...
	cStr ToStr(const int nPrec = 2) const;
};
...
// cAngles::ToPtr : const float * () const
inline const float * cAngles::ToPtr() const {
	return &Pitch;
}

// cAngles::ToPtr : float * ()
inline float * cAngles::ToPtr() {
	return &Pitch;
}