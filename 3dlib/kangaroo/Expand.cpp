...
bool ExpandSurface::Start(PolyMesh* M,DWORD Face,int x,int y){
    Stack1.FastClear();
    Stack2.FastClear();
    Result.FastClear();	
	UsedInsFaces.clear();
            ...
            for(int j=0;j<4;j++){
                ...
                if(!Used.get(DW)){
                    ...
                    if(w>0.01){
						...
						npoints++;
						if(npoints>200000){
							if(inquest && IDNO==MessageBox(IRS->GetHWND(),TextManager::GetText("QUEST_LONG_FILL"),TextManager::GetText("FILL_WARNING"),MB_ICONQUESTION|MB_YESNO)){								
								Stack1.FastClear();
								Stack2.FastClear();    
								Used.clear();
								UsedInsFaces.clear();								
