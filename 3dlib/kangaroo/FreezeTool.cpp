...
float FreezeTool::GetFreezeAmount(ModPointInfo* mp){
    ...
    if(FreezeMode){
        if(FreezeMode<3){
            ...
            float dp=N.dot(avg-mp->Base->Pos);
...
void FreezeTool::ModifyPoint(float scale,ModPointInfo& pi,float R,float R1 ){
    if(InvFreeze){
        ...
        f*=m;
        f=1-f;
        pi.Base->Freeze=min(pi.Base->Freeze,f);
    }else{
        float m=(1-sqrt(pi.dx*pi.dx+pi.dy*pi.dy))*6;
        if(m>1)m=1;
        float f=GetFreezeAmount(&pi);
        ...
        if(f>1)f=1;
        f*=m;
        pi.Base->Freeze=max(pi.Base->Freeze,f);
    }
}
...
void FreezeTool::UnFreeze(float scale,ModPointInfo& pi,float R,float R1 ){
    float m=(1-sqrt(pi.dx*pi.dx+pi.dy*pi.dy))*6;
    if(m>1)m=1;
    pi.Base->Freeze=min(pi.Base->Freeze,1-m);
}
...
bool FreezeTool::opInvertFreeze(){
    InvertFreeze();
	return true;
}