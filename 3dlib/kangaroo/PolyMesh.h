...
struct FaceInfo{
    ...
    BYTE        Complexity;
    SurfPoint3D* Points;
    DynArray<LayerOnFace> Layers;
};
class OneLayerInfo{
public:
    int   LayerID;
    bool  Visible;
    float DepthTransparency;
...
typedef void fnVertModificator(Vector3D& pos,float& SmoothDegree,Vector3D AvgNormal,PolyMesh* M);

class SurfaceModificator{
public:		
	SurfaceModificator();
	static uni_hash<VertexBackup> Backup;	
    static uni_hash<Vector3D*>    SculptHash;
    static float                    PrevR;
    ...
    static Vector3D					PrevPrevPosS[8];
	static Vector3D					PrevPosS[8];
	static Vector3D					CurrPosS[8];
    static Vector3D					CenterPosS[8];	

	static PickInfo*				Pick;
    static TexArray*                TexPick;
	static PolyMesh*				M;	
    static Vector3D                 LastNormal;
    ...
    static Vector3D CreateVertList();
    void  CreateTexPick();
	virtual void Apply(){};
    ...
    static void ApplySculptModification(fnVertModificator* fn,float SmoothDegree);
    static void ClearSculptHash();
};
...
public:
    ...
    uni_hash<OneFaceBackup*,bi_DWORD>       FacesHistory;	
    DynArray<OneLayerInfo*>                 Layers;
    int                                     CurrentLayerID;

    //layers operations - see layers.cpp
    int                             AddLayer(bool MakeActive);
    void                            DeleteLayer(int LayerID);
    void                            SetVisible(int LayerID,bool Visible);
    bool                            GetVisible(int LayerID);
    void                            SetGeometryTransparency(int LayerID,float T);
    void                            SetColorTransparency(int LayerID,float T);
    void                            SetActiveLayer(int LayerID);
    void                            CheckFaceOnEmptyness(int FaceID,int LayerID);
    LayerOnFace*                    GetCurrentLayer(int FaceID);
    LayerOnFace*                    GetSomeLayer(int FaceID,int LayerID);
    void                            InvalidateLayer(int LayerID);
    OneLayerInfo*                   GetLayer(int LayerID);
    void                            CalcFaceAttributesWithLayers(int Face);
    //end
	...
	void							ApplyCatmullClark	(DWORD SubdLevel);
