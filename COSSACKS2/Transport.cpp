...
char GetBestDirection(MotionField* MFI,int xd,int yd,int LX,char CurDir){
    ...
    do{
	    ...
	    for(int D0=0;D0<256;D0+=16){
		    int DY=((L0+DD)*TSin[D0])/256;
		    int DX=((L0+DD)*TCos[D0])/256;
...
bool SearchTransportPlace(int* x,int* y,int Lx){
	...
	for(int r=0;r<40;r++){
		...
		for(int p=0;p<N;p++){
			int xx=x0+xi[p];
			int yy=y0+yi[p];
			if(!MFIELDS[1].CheckRound(xx-3,yy-3,Lx+6)){
				*x=xx;
				*y=yy;
				return true;
			};
...
bool ParkWaterNewMonster(OneObject* OB,int x,int y,byte Prio,byte OrdType){
	...
	int yy1=yy;
	if(OB->GLock)MFI->ClearRound(OB->x,OB->y,LX);
	if(!MFI->CheckRound(xx1-1,yy1-1,LX+2)){
		if(OB->GLock)MFI->SetRound(OB->x,OB->y,LX);
		return false;
	};
...
bool CheckIfCostIsNear(int x,int y){
	for(int i=10;i<14;i++){
		char* xi=Rarr[i].xi;
		char* yi=Rarr[i].yi;
		int N=Rarr[i].N;
		for(int j=0;j<N;j+=3){
            if(!CheckPt(x+xi[j],y+yi[j]))return true;
		}
	}
	return false;
}
bool CheckIfCostIsNearOB(int x,int y,OneObject* OB){
    ...
    for(int i=-32;i<=32;i+=8){
		byte DIR=OB->RealDir+i;
        int xx=x+(TCos[DIR]*L)/256;
        int yy=y+(TSin[DIR]*L)/256;
        if(GetHeight(xx,yy)>1)nh++;        
        if(nh)return true;
    }
    return nh;
}
void WaterNewMonsterSendToLink(OneObject* OB){
	...
	if(tt){
		...
		if(abs(ddir)<=MRot){
			...
			int D=MRot*GameSpeed;
			if(D>512)D=512;
			if(ddir>0)OB->RealDirPrecise+=D;
			else OB->RealDirPrecise-=D;
		...
		if(distan<8&&CheckIfCostIsNearOB(OB->RealX>>4,OB->RealY>>4,OB)){
			OB->DeletePath();
			OB->DestX=-1;
...
void TransProcess::AddUnit(word UnitID){
	...
	NewMonster* NM=OB->newMons;
	if(OB->LockType||NM->Building)return;
...
void TransProcess::Process(){
	...
	if(TransportID!=0xFFFF){
		...
		AdvCharacter* ADC=TRA->MoreCharacter;
...
void FishingLink(OneObject* OBJ){
	...
	if(OBJ->Ustage>=ADC->FishAmount){
		...
		AdvCharacter* NM=OBJ->MoreCharacter;
