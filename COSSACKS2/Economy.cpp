...
void SelectBUYBUTTON(int i){
	if(ECO.ResSell!=-1&&ECO.ResBuy!=-1){
		CmdTorg(GSets.CGame.cgi_NatRefTBL[MyNation],ECO.ResSell,ECO.ResBuy,ECO.NeedToSell);
	};
};
...
void Economy::CreateInterface(IconSet* ISET){
	static int GPFile=-1;
	if(GPFile==-1){
		GPFile=GPS.PreLoadGPImage("IconsOld");
		//if(GPS.GPNFrames(GPFile)==0x7FFFFFFF) GPFile=0;
	}
	for(int i=0;i<6;i++)if(i!=3){
		int ix=i%2;
		int iy=i>>1;
		OneIcon* IC=ISET->AddIconFixed(GPFile,EcResIcn[i],ix+iy*12);
		if(ResSell==i){
			IC->SelectIcon();
		};
		IC->AssignLeft(&SelectSELL,i);
		IC->CreateHint(RDS[EcResID[i]].Name);
		IC=ISET->AddIconFixed(GPFile,EcResIcn[i],ix+7+iy*12);
		if(ResBuy==i){
			IC->SelectIcon();
		};
