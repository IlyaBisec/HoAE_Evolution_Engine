...
inline int dMatrix::Dim(int nDim) {
	cAssert(cMath::IsInRange(nDim, 2, 4));
...
inline dMatrix::dMatrix(const dMatrix &R, const dVector &T) {
	cAssert(R.Dim() == T.Dim() && (R.Dim() == 2 || R.Dim() == 3));
	Copy(R);
	Dim(m_Dim + 1);
...
inline double & dMatrix::operator () (int row, int col) {
	cAssert(row >= 0 && row < m_Dim && col >= 0 && col < m_Dim);
	return m[row][col];
} // dMatrix::operator ()
...
inline bool dMatrix::IsZero(const double Epsilon) const {
	switch(m_Dim) {
		...
		default: cAssert(m_Dim >= 2 && m_Dim <= 4);
			return false;
	}
} // dMatrix::IsZero
