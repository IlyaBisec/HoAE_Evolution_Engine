...
bool testBooleans3D() {
/*	cRawMesh rm;
	rm.LoadObj("Meshes\\Bridge.obj");
	rm.Type(comms::RMTT_TRIS);
	rm.VERTS.ScaleTo(300);
	SurfMesh sm;
	Convert(rm, sm);
	SubdivideArgs Args;
	Args.fAdaptive = true;
	...
	Args.Scale = 1.2;
	sm.CalcNormals();
	while(sm.Subdivide(Args));
	sm.Relax(0.2, 1);
	Convert(sm, rm);
	rm.SaveX("SubDiv.x");
	rm.SaveObj("SubDiv.obj");*/
	return false;

//	Ref.Subdivide(30);
/*	double r, Phi, Psi;
	cVector c = cVector::Cartesian(1, comms::Rad(30), comms::Rad(-15));
	c.Spherical(r, Phi, Psi);
	Phi = comms::Deg(Phi);
	Psi = comms::Deg(Psi);
	//------------------------------------
	c = cVector::Cartesian(1, comms::Rad(100), comms::Rad(-30));
	c.Spherical(r, Phi, Psi);
	Phi = comms::Deg(Phi);
	Psi = comms::Deg(Psi);
	//----------------------------------------
	c = cVector::Cartesian(1, comms::Rad(200), comms::Rad(-45));
	c.Spherical(r, Phi, Psi);
	Phi = comms::Deg(Phi);
	Psi = comms::Deg(Psi);
	//----------------------------------------
	c = cVector::Cartesian(1, comms::Rad(300), comms::Rad(-45));
	c.Spherical(r, Phi, Psi);
	Phi = comms::Deg(Phi);
	Psi = comms::Deg(Psi);*/
	//---------------------------------------------
...
/*	SurfMesh vbm;
	vbm.GenMarker(SurfVertex::Origin, 10, false);
