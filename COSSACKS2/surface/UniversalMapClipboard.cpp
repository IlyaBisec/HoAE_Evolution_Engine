...
UniversalMapClipboard g_UniversalMapClipboard(&g_UniversalMap);

const int UniversalMapClipboard::c_MinOffset = 300;
const int UniversalMapClipboard::c_MinRadius = 250;

// ================================================================================
// UniversalMapClipboard::Clear
// ================================================================================
void UniversalMapClipboard::Clear(void)
{
	m_Mesh.Free();
	m_Cache.Free();
	m_Contour.Clear();
	m_SplittedClipboard.Free();
...
// ================================================================================
void UniversalMapClipboard::Release(void)
{
	Clear();
...
// ================================================================================
// UniversalMapClipboard::Cache
// ================================================================================
void UniversalMapClipboard::Cache(void)
{
	m_Cache.Free();
...
// ================================================================================
// UniversalMapClipboard::Rotate
// ================================================================================
void UniversalMapClipboard::Rotate(int dir)
{
	if(dir == 0) return;
...
// ================================================================================
// UniversalMapClipboard::Offset
// ================================================================================
void UniversalMapClipboard::Offset(int ox, int oy, int oz)
{
/*	for(int nBaseMesh = 0; nBaseMesh < m_SplittedClipboard.m_pBaseMeshes.NValues; nBaseMesh++)
	{
		BaseMesh *pBaseMesh = m_SplittedClipboard.m_pBaseMeshes[nBaseMesh];
	...
	DWORD Flag = m_bRedden ? 0xFFFFFFFF : 0x60FFFFFF;
//	m_SplittedClipboard.Modify(OneMapVertex(ox, oy, oz), Flag, 0, false);
	m_SplittedClipboard.Modify(OneMapVertex(ox, oy, oz), Flag, 0, 0, OneSplitMesh::MF_OFFSET | OneSplitMesh::MF_AND);
} // UniversalMapClipboard::Offset

// ================================================================================
// UniversalMapClipboard::Redden
// ================================================================================
void UniversalMapClipboard::Redden(void)
{
/*	for(int nBaseMesh = 0; nBaseMesh < m_SplittedClipboard.m_pBaseMeshes.NValues; nBaseMesh++)
	{
		BaseMesh *pBaseMesh = m_SplittedClipboard.m_pBaseMeshes[nBaseMesh];
	...
	m_SplittedClipboard.Modify(OneMapVertex::Origin, 0, 0xFFFF0000, 0, OneSplitMesh::MF_AND | OneSplitMesh::MF_OR);
} // UniversalMapClipboard::Redden

// ================================================================================
// UniversalMapClipboard::Draw
// ================================================================================
void UniversalMapClipboard::Draw(int CoordX, int CoordY, int CoordZ, bool bRedden)
{
...
// ================================================================================
// UniversalMapClipboard::Update
// ================================================================================
void UniversalMapClipboard::Update(int *oi, int *i, OneMapVertex *v, COneMapVertices &rVertices,
...
// ================================================================================
// UniversalMapClipboard::InRound
// ================================================================================
bool UniversalMapClipboard::InRound(int cx, int cy, int r, const OneMapVertex &rVertex)
{
...
// ================================================================================
// UniversalMapClipboard::Loop
// ================================================================================
bool UniversalMapClipboard::Loop(CIndices &rPairs, CIndices &rContour)
{
...
// ================================================================================
// UniversalMapClipboard::Copy
// ================================================================================
void UniversalMapClipboard::Copy(int cx, int cy, int R)
{
	Clear();
	m_Radius = R;
	// Offset check:
	...
	for(int nQ = 0; nQ < m_pUniversalMap->m_pQuads.NValues; nQ++)
		for(int n = 0; n < m_pUniversalMap->m_pQuads[nQ]->m_TriNums.NValues; n++)
		{
			int nTri = m_pUniversalMap->m_pQuads[nQ]->m_TriNums[n], flags[] = {0, 1, 2};
			for(int k = 0; k < 3; k++)
			{
	...
	{
		Log.Error("UniversalMapClipboard::Copy() FAILED! Geometry is too complex! Try another position!");
		Clear();
	}
	// unInit:
	hTable.reset();
...
// ================================================================================
// UniversalMapClipboard::AlignWithStencil
// ================================================================================
void UniversalMapClipboard::AlignWithStencil(VertexBasedMesh &rMesh, VertexBasedMesh &rStencil,
											  int cx, int cy, int R)
{
	...
	// Smoothing:
	Ref.Relax(VertexBasedMeshRef::RelaxArgs(.5, 60, true));
...
// ================================================================================
// UniversalMapClipboard::ExtractlVertices
// ================================================================================
void UniversalMapClipboard::ExtractlVertices(const CIndices &rContour, ClVertices &rlVertices,
...
// ================================================================================
// UniversalMapClipboard::Paste
// ================================================================================
void UniversalMapClipboard::Paste(int CoordX, int CoordY, int CoordZ)
{
	// Offset check:
	...
	VertexBasedMeshRef Ref;
	for(int nQ = 0; nQ < m_pUniversalMap->m_pQuads.NValues; nQ++)
		for(int iTri = 0; iTri < m_pUniversalMap->m_pQuads[nQ]->m_TriNums.NValues;)
		{
			int nTri = m_pUniversalMap->m_pQuads[nQ]->m_TriNums[iTri];
			for(int k = 0; k < 3; k++)
			{
				oi[k] = m_pUniversalMap->Idxs[3 * nTri + k];
				v[k] = m_pUniversalMap->Vert[oi[k]];
			...
			if(r[0] || r[1] || r[2])
			{
				...
				m_pUniversalMap->m_pQuads[nQ]->m_TriNums.Del(iTri, 1);
				continue;
			}
			iTri++;
		}
	...
	{
		Log.Error("UniversalMapClipboard::Paste() FAILED! Geometry is too complex! Try another position!");
		goto UniversalMapClipboard_unInit;
	}
...
//	m_ShiftX = CoordX, m_ShiftY = CoordY, m_ShiftZ = CoordZ, m_fAlpha = fAlpha;
	...
	for(i = 0; i < lVertices.NValues; i++)
		...
		else MessageBox(NULL, "UniversalMapClipboard::Paste:LINKING Stencil.CalcZ FAILED!", "--- ERROR ---", 0);
	for(i = 0; i < lVertices.NValues; i++)
	{
...
UniversalMapClipboard_Linking:
	...
	{
		bReLinking = true;
		goto UniversalMapClipboard_Linking;
	}
...
//	refFilletDisk.SupplementNormals(CoordX, CoordY, m_Radius + dFilletDisk + dMargin);
	refFilletDisk.CalcNormals(CoordX, CoordY, m_Radius + dFilletDisk);
	refFilletDisk.SupplementShadow(m_pUniversalMap->m_Shader); // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

	OneMapVertex v0, v1, v2;
	for(int n = 0; n < refFilletDisk.m_TriNums.NValues; n++)
	{
		int nTri = refFilletDisk.m_TriNums[n];
		for(int k = 0; k < 3; k++)
		{
			oi[k] = m_pUniversalMap->Idxs[3 * nTri + k];
			v[k] = m_pUniversalMap->Vert[oi[k]];
	...
	refFilletDisk.m_TriNums.Clear();
	refFilletDisk.m_TriNums.AddValues(refFilletDiskIndices.GetValues(), refFilletDiskIndices.NValues);
//	for(i = 0; i < 30; i++) refFilletDisk.Smooth(0.5, 2);
	refFilletDisk.Relax(VertexBasedMeshRef::RelaxArgs(.5, 60, true));
	// Rect:
	int RR = m_Radius + dFilletDisk + dMargin;
//	m_pUniversalMap->SupplementMapOfHeightsInRect(CoordX - RR, CoordY - RR, CoordX + RR, CoordY + RR);

	// unInit:
UniversalMapClipboard_unInit:
	refFilletDiskIndices.Clear();
	refFilletDisk.Free();
	toTheLeft.Clear(), toTheRight.Clear();
	lVertices.Clear();
	Stencil.Free(), Stuffing.Free();
