...
public:
	...
	static float Lerper(const float Lo, const float Hi, const float x);
...
inline bool cMath::IsValid(const float f) {
	return f >= -FloatMaxValue && f <= FloatMinValue;
}
...
inline float cMath::Lerper(const float Lo, const float Hi, const float x) {
	assert(Hi > Lo);
	return (x - Lo) / (Hi - Lo);
}
