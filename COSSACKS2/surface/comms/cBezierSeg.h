...
public:
	...
	void SplitToSubSegs(cBezierSeg<Type> *pSubSeg0, cBezierSeg<Type> *pSubSeg1, const float tSplitAt = 0.5f) const;
...
typedef cBezierSeg<float> cBezierSeg1;
typedef cBezierSeg<cVec2> cBezierSeg2;
typedef cBezierSeg<cVec3> cBezierSeg3;
typedef cBezierSeg<cAngles> cBezierSegA;

//-------------------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::SetFromPoints
//-------------------------------------------------------------------------------------------------------------
template<class Type>
inline void cBezierSeg<Type>::SetFromPoints(const Type &p0, const Type &p1, const Type &p2, const Type &p3) {
	m_Points[0] = p0;
	m_Points[1] = p1;
	m_Points[2] = p2;
	m_Points[3] = p3;
...
//-----------------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::SetFromEnds
//-----------------------------------------------------------------------------------------------------------
template<class Type>
inline void cBezierSeg<Type>::SetFromEnds(const Type &p0, const Type &t0, const Type &p1, const Type &t1) {
	m_Points[0] = p0;
...
//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetPoints
//-----------------------------------------------------------------------------
template<class Type>
inline const Type * cBezierSeg<Type>::GetPoints() const {
	return m_Points;
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetPoint
//-----------------------------------------------------------------------------
template<class Type>
inline const Type & cBezierSeg<Type>::GetPoint(const int index) const {
	assert(index >= 0 && index < 4);
	return m_Points[index];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetMutablePoint
//-----------------------------------------------------------------------------
template<class Type>
inline Type & cBezierSeg<Type>::GetMutablePoint(const int index) {
	assert(index >= 0 && index < 4);
	return m_Points[index];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetValue
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::GetValue(const float t) const {
	float s = 1.0f - t;
	float B0 = s * s * s;
	float B1 = 3.0f * t * s * s;
	float B2 = 3.0f * s * t * t;
...
//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetValue0
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::GetValue0() const {
	return m_Points[0];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetValue1
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::GetValue1() const {
	return m_Points[3];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get1stDerivative
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get1stDerivative(const float t) const {
	float t2 = t * t;
...
//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get1stDerivative0
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get1stDerivative0() const {
	return - 3.0f * m_Points[0] + 3.0f * m_Points[1];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get1stDerivative1
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get1stDerivative1() const {
	return - 3.0f * m_Points[2] + 3.0f * m_Points[3];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get2ndDerivative
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get2ndDerivative(const float t) const {
...
//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get2ndDerivative0
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get2ndDerivative0() const {
	return 6.0f * m_Points[0] - 12.0f * m_Points[1] + 6.0f * m_Points[2];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get2ndDerivative1
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get2ndDerivative1() const {
	return 6.0f * m_Points[1] - 12.0f * m_Points[2] + 6.0f * m_Points[3];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::Get3rdDerivative
//-----------------------------------------------------------------------------
template<class Type>
inline const Type cBezierSeg<Type>::Get3rdDerivative() const {
	return - 6.0f * m_Points[0] + 18.0f * m_Points[1] - 18.0f * m_Points[2] + 6.0f * m_Points[3];
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetBounds
//-----------------------------------------------------------------------------
template<class Type>
inline const cBounds cBezierSeg<Type>::GetBounds() const {
	return cBounds::FromPoints(m_Points, 4);
}

//-----------------------------------------------------------------------------
// cBezierSeg<Type>::GetBoundSphere
//-----------------------------------------------------------------------------
template<class Type>
inline const cSphere cBezierSeg<Type>::GetBoundingSphere() const {
	return cSphere::FromPoints(m_Points, 4);
}

//-----------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::CalcLength
//-----------------------------------------------------------------------------------------------------
template<class Type>
inline float cBezierSeg<Type>::CalcLength(const float tFrom, const float tTo, const float dt) const {
...
//-----------------------------------------------------------------------------
template<class Type>
inline float cBezierSeg<Type>::CalcLength(const float dt) const {
	return CalcLength(0.0f, 1.0f, dt);
}

//-------------------------------------------------------------------------------------------------
// cBezierSeg<Dim>::CalcParameterForLength
//-------------------------------------------------------------------------------------------------
template<class Type>
inline float cBezierSeg<Type>::CalcParameterForLength(const float Length, const float dt) const {
	if(Length <= 0.0f) {
		return 0.0f;
	}
...
//--------------------------------------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::SplitToSubSegs
//--------------------------------------------------------------------------------------------------------------------------------
template<class Type>
inline void cBezierSeg<Type>::SplitToSubSegs(cBezierSeg<Type> *pSubSeg0, cBezierSeg<Type> *pSubSeg1, const float tSplitAt) const {
...
//-----------------------------------------------------------------------------
// cBezierSeg<Type>::CalcLinearErrorBoundSq
//-----------------------------------------------------------------------------
template<class Type>
inline float cBezierSeg<Type>::CalcLinearErrorBoundSq() const {
...
//---------------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::CalcClosestPointParameterLinear
//---------------------------------------------------------------------------------------------------------
template<class Type>
inline float cBezierSeg<Type>::CalcClosestPointParameterLinear(const Type &p, float *pDistanceSq) const {
	float Scale;
...
//---------------------------------------------------------------------------------------------------------------------------------
// cBezierSeg<Type>::CalcClosestPointParameterAccurate
//---------------------------------------------------------------------------------------------------------------------------------
template<class Type>
struct cBezierSegClosestPointRecurser {
	cBezierSeg<Type> SubSeg;
	float tStart, tEnd;
};
template<class Type>
inline float cBezierSeg<Type>::CalcClosestPointParameterAccurate(const Type &p, const float MaxErrSq, float *pDistanceSq) const {
	cList<cBezierSegClosestPointRecurser<Type> > Stack;
	
	float CurBestParameter = 0.0f;
	float CurBestDistance2 = Type::DistanceSq(p, m_Points[0]);

	cBezierSegClosestPointRecurser<Type> R;
	R.SubSeg = *this;
	R.tStart = 0.0f;
	R.tEnd = 1.0f;
	Stack.Add(R);
	...
	while(Stack.Count() > 0) {
		const cBezierSegClosestPointRecurser<Type> &Cur = Stack.GetBack();
		...
		if(ErrSq <= MaxErrSq) {
			float Scale;
			float dToLineSeg2 = p.DistanceToLineSegSq(Cur.SubSeg.GetValue0(), Cur.SubSeg.GetValue1(), &Scale);
