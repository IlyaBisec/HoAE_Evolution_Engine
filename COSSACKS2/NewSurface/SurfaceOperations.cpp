...
void VertexBasedMeshRef::ReturnVertrex(DynArray<VecPr>& MasOut)
{
	...
	for (int i=0; i<m_TriNums.GetAmount(); i++)
	{	
...
{
	int Mnum=Mref.m_TriNums.GetAmount();
	int Mtri=Tri.GetAmount();
	for (int i=0; i<Mnum; i++)
	{
		Triang& t=Tri[i];
		int ind=Mref.m_TriNums[i];
		for (int i0=0; i0<3; i0++)		
...
{	
	int Ntri=Mref.m_TriNums.GetAmount();
	Tri.Clear();
	Triang t;
	Vector3D a;
	t.Finish=0; 
		...
		for (int xi=x1; xi<=x2; xi++)
			for (int yi=y1; yi<=y2; yi++)
				if (!(xi>=ox1 && xi<=ox2 && yi>=oy1 && yi<=oy2))
				{
					...
					int Ntri=UniMap.m_pQuads[ind]->m_TriNums.GetAmount();
					for (int i=0; i<Ntri; i++)
						Mref.m_TriNums.Add(UniMap.m_pQuads[ind]->m_TriNums[i]);
				}
		ox1=x1;
		ox2=x2;
		oy1=y1;
		...
		for (int xi=x1; xi<=x2; xi++)
			for (int yi=y1; yi<=y2; yi++)
				if (!(xi>=ox1 && xi<=ox2 && yi>=oy1 && yi<=oy2))
				{
					...
					for (int i=0; i<Ntri; i++)
					{
						int nt=UniMap.m_pQuads[ind]->m_TriNums[i];
...
void UniversalMap::ModifySurface(int x1, int y1, int x2, int y2, int MaxSubDiv,BaseSurfaceFunction* Func)
{
	...
	for (int xi=xi0; xi<=xi1; xi++)
		for (int yi=yi0; yi<=yi1; yi++)
		{
			...
			q.Dirty=true;
			for (int i=0; i<q.m_TriNums.GetAmount(); i++)
				ref.m_TriNums.Add(q.m_TriNums[i]);
			q.m_TriNums.Clear();
		}
		int Nind=ref.pOrigin->Idxs.GetAmount();
		int Ntri=ref.m_TriNums.GetAmount();
		ref.ModifySurface(MaxSubDiv,Func);		
		for (int i0=0; i0<Ntri; i0++)
		{
			int i=ref.m_TriNums[i0];
			OneMapVertex& v0=Vert[Idxs[3*i]];
			OneMapVertex& v1=Vert[Idxs[3*i+1]];
			OneMapVertex& v2=Vert[Idxs[3*i+2]];
			OneMapVertex v;
...
void CreateNew3DMap(int size,DWORD Color){
	...
	g_UniversalMap.Refresh(SSLocalIllumination);
	InitLayersSystem();
}
///-----------------MAIN FUNCTION-----------------///
bool MathTestDraw()
