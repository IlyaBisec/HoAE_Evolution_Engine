...
LRESULT CScriptDebugger::DebugMessage(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case DMSG_ADD_LOCALVARIABLE:{
			...
			msg.w_buff((void*)wParam,sizeof(Variable_dbg));
			SendMessageToIde(msg);
		}break;
...
void CScriptDebugger::AddLocalVariable(const Variable_dbg& var)
{
/*	Variable var;
...
void CScriptDebugger::Eval(const char* strCode, char* res)
{
	...
	strCodeFull[0] = 0;
	const char * r = "return  ";
	strcat(strcat(strCodeFull,r),strCode);
