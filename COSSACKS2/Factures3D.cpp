...
bool Exec(const char *pFileName, const char *pConsoleOutputFile, const char *pCmdArgs = NULL) {
	assert(pFileName);
	assert(pConsoleOutputFile);

	HANDLE hStdout = GetStdHandle(STD_OUTPUT_HANDLE);
	HANDLE hChildStdoutRd, hChildStdoutWr, hChildStdoutRdDup, hChildStdoutWrDup;
	...
	if(!DuplicateHandle(GetCurrentProcess(), hChildStdoutRd, GetCurrentProcess(),
		&hChildStdoutRdDup, 0, FALSE, DUPLICATE_SAME_ACCESS)) return false;
	...
	if(pCmdArgs) {
		sprintf(Str, "\"%s\" %s", pFileName, pCmdArgs);
	}
	if(!CreateProcess(NULL, Str, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
		CloseHandle(pi.hProcess);
		CloseHandle(pi.hThread);
		return false;
	}
	
	HANDLE hFile = CreateFile(pConsoleOutputFile, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	while(true) {
		if(!ReadFile(hChildStdoutRdDup, Buffer, sizeof(Buffer), &dwRead, NULL) || dwRead == 0) break;
		if(!WriteFile(hFile, Buffer, dwRead, &dwWritten, NULL)) break;
	}
...
void LoadFactures(){
	...
	if(CheckIfFileExists("Surface\\FacturesList.xml")){
        ...
        //////////////////////////////////////////////////////////////////////////        
        //for(int i=0;i<FacturesList.GetAmount();i++){
        //    char cc[32];
        //    sprintf(cc,"Temp\\texg2\\%03d.dds",i*2);
        //    CopyFile(IRS->GetTextureName(FacturesList[i]->FactureID),cc,false);
        //    sprintf(cc,"Temp\\texg2\\%03d.dds",i*2+1);
        //    CopyFile(IRS->GetTextureName(FacturesList[i]->FactureID),cc,false);
        //}
	}else{
		//loading textures
...
void RoadsSystem::DrawVisibleRoads(){
	...
	float sc=1.0;
	float scy=1.0;
	static int shadow=IRS->GetTextureID("ShadowMap");
	...
	static int rt_mul=IRS->GetShaderID("road_mul");
	static int rt_dot=IRS->GetShaderID("road_dot");
	static int rt_nrm=IRS->GetShaderID("road");

	IRS->SetShader(rt_nrm);

	for(int q=0;q<NRoads;q++){
		if(CheckObjectVisibility(R->xc,R->yc,R->zc,R->R)){
			if(R->BM){
				IRS->SetTexture(shadow,2);
				DrawBM(*R->BM);
			}else{
				int N=R->Np;
				if(N>0){
...
void ProcessNationOfRoads(){
	if(!MOptions.AutoChangeRoadsNation)return;
	bool Changed=false;
	for(int i=0;i<RN.NKnots;i++){
...
void HideRoadsInRadius(int x,int y,int R,bool Hide){
    OneNetKnot* NK=RN.Net;
    bool ch=false;
    for(int i=0;i<RN.NKnots;i++){
        if(Norma(NK->x-x,NK->y-y)<R){
            NK->Hidden=Hide;
            ch=true;
        }
        NK++;        
