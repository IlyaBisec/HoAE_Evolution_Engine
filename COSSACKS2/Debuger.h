...
public:
	...
	SAVE(be_CDebuger);
		...
		REG_METHOD_EX(FN_SWITCH_VISIBLE,_SWITCH_VISIBLE);

		REG_AUTO(_curSRC);

		REG_MEMBER(_bool,_SCRIPT_TXT);
		REG_MEMBER(_bool,_SCRIPT_SRC);
		REG_METHOD_EX(FN_SCRIPT_SWICHER,_SCRIPT_SWICHER);
		REG_MEMBER(_bool,_updateAlways);

		REG_AUTO(_SCRIPT_TXT_LIST);
		REG_METHOD_EX(FN_UPDATE_TXT,_UPDATE_TXT);
		REG_METHOD_EX(FN_UPDATE_TXT_SEL,_UPDATE_TXT_SEL);

		REG_METHOD_EX(FN_MOVE_SELECTOR,_MOVE_SELECTOR);
		
		REG_AUTO(_strSearch);
		REG_METHOD_EX(FN_SEARCH_STR_SWITCH_ACTIVITY,_SEARCH_STR_SWITCH_ACTIVITY);
		REG_MEMBER(_bool,_strSearchActive);
		REG_METHOD_EX(FN_SEARCH,_SEARCH);

		REG_METHOD_EX(FN_RIGHT_CLICK,_RIGHT_CLICK);
		NOSAVE	REG_MEMBER(_bool,_nextCall);
		REG_METHOD_EX(FN_NEXT_CALL,_NEXT_CALL);
		NOSAVE	REG_MEMBER(_bool,_interrupt);
		REG_METHOD_EX(FN_INTERRUPT,_INTERRUPT);
		REG_MEMBER(_bool,_smartBP);
		REG_AUTO(_SMARTBP_LIST);
		REG_METHOD_EX(FN_DELETE_SMARTBP,_DELETE_SMARTBP);
		REG_METHOD_EX(FN_ADD_EDIT_SMARTBP,_ADD_EDIT_SMARTBP);

		REG_METHOD_EX(FN_VARIABLES,_VARIABLES);
		REG_METHOD_EX(FN_FUNCTIONS,_FUNCTIONS);
		REG_MEMBER(_bool,_watch);
		REG_AUTO(_WATCH_LIST);
		REG_METHOD_EX(FN_DELETE_WATCH,_DELETE_WATCH);
		REG_METHOD_EX(FN_ADD_EDIT_WATCH,_ADD_EDIT_WATCH);

		REG_AUTO(_FILE_NAME);
		REG_METHOD_EX(FN_FILE_DIALOG,_FILE_DIALOG);
		REG_METHOD_EX(FN_SAVE,_SAVE);
		REG_METHOD_EX(FN_LOAD,_LOAD);
