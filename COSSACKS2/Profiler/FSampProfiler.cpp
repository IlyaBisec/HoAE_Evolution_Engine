...
FSampProfiler::FSample FSampProfiler::LastFrameSamples[MaxBreaksOnFrame];
int FSampProfiler::LastFrameSamplesAmount=0;
int FSampProfiler::LastFrameTime=0;
		...
		if(GetKeyState(VK_NUMLOCK) & 1)
		{
			...
			if(CurrFunctionIdx != -1)
			{
				...
				if(Idx != -1)
				{
					// - нашли путь
					//--by drew --// 
					//improvement is that we collect all samples for one frame and if there was lags, we insert them into profiler structures
					if(LastFrameSamplesAmount<MaxBreaksOnFrame){
						FSample& smp=LastFrameSamples[LastFrameSamplesAmount++];
						smp.Count = 0;
...
//------------------------------------------------------------------------------------------------------
bool FSampProfiler::OnFrame(int MinTimeDiff){	
	EnterCriticalSection(&cs);
	bool smt=false;
	int T=GetTickCount();
	if(LastFrameTime==0)LastFrameTime=T;
	if(T-LastFrameTime>MinTimeDiff){
		for(int i=0;i<LastFrameSamplesAmount;i++){
			FTernTreeNode* t = Insert(LastFrameSamples+i);
			t->Sample->Count++;
			smt=true;
		}		
	}
	LastFrameSamplesAmount=0;
	LastFrameTime=T;
	LeaveCriticalSection(&cs);
	return smt;
}
