		...
		for(int i=0;i<NW;i++){
			...
			if(!strcmp(OSW->WinID,win)){
				...
				if(F!=INVALID_HANDLE_VALUE){
					...
					if(NEWST[0]==13 && NEWST[1]==10){
                        strcpy(NEWST,NEWST+2);
					}
					if( NEWST[0]=='<'&&OSW->Result ){
						//replacing block
						int L00,LNS;
...
// MANOWAR
// static ClassArray<_str> PFileList;//new 17.05

static ClassArray<_str> * PFileList = NULL;
		...
		do{
			if(strcmp(FD.cFileName,".") && strcmp(FD.cFileName,"..")){
				if(FD.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY){
					...
					new_root.Add("\\");
					ReadDir(Pass0,new_root.pchar(),F0,F);
				}else{
					DString file0;
					file0.Add(Pass0);
					file0.Add(Pass);
					...
					//new 17.05
					if ( ! PFileList )
						PFileList = new ClassArray<_str>;
					int N=PFileList->GetAmount();
					_str* ss=new _str;
					...
					do{
						fin=true;
						// bred kakoy-to
						for(int i=0;i<N;i++)if(!strcmp((*PFileList)[i]->pchar(),ss->pchar())){
							NF++;
							char ff[128];
							sprintf(ff,"%s{%d}",FD.cFileName,NF);
							ss->Assign(ff);
							fin=false;
							break;
						}
...
CIMPORT void RenameAnywayEx(char* src,char* dst);
void exec_LW_dff(int Np,char** par,int* size){
	if(Np>=2&&size[1]>=8){
		DWORD* DD=(DWORD*)par[1];
		int ofs=DD[0];
